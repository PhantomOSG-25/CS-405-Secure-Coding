...
std::string read_file(const std::string& filename)
{
    ...
    // Stop the program with a clear message if the file cannot be opened.
    if (!input_file.is_open())
    {
        throw std::runtime_error(
            "The input file could not be opened: " + filename);
    }
    ...
    const std::string& data)
{
    ...
    {
        throw std::runtime_error(
            "The output file could not be created: " + filename);
    }
    ...
    // Save the information in the required format.
    output_file << student_name << '\n';
    output_file << std::put_time(&local_time, "%Y-%m-%d") << '\n';
    output_file << key << '\n';
    output_file << data;

    // Close the file after saving the information.
    output_file.close();
}
